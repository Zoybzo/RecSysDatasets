...
class YELPDataset(BaseDataset):
    def __init__(self, input_path, output_path):
        ...
        self.photo_file = os.path.join(
            self.input_path, "photos.json"
        )
        ...
        self.output_item2text_file = os.path.join(self.output_path, self.dataset_name + 'item2text')
        self.output_photos_file = os.path.join(self.output_path, self.dataset_name + 'photos')
    ...
    def convert_photos(self):
        fin = open(self.photo_file, "r")
        fout = open(self.output_photos_file, "w")
        ...
        fout.write(
            "\t".join(
                [self.photo_fields[column] for column in self.photo_fields.keys()]
            )
        )
        ...
        for _ in tqdm(range(lines_count)):
            ...
            if line_dict['business_id'] not in self.item_map:
                self.item_map[line_dict['business_id']] = len(self.item_map) + 1
            line_dict['business_id'] = self.item_map[line_dict['business_id']]
            ...
            fout.write(
                "\t".join(
                    [
                        str(
                            line_dict[
                                self.photo_fields[key][
                                    0 : self.photo_fields[key].find(":")
                                ]
                            ]
                        )
                        for key in self.photo_fields.keys()
                    ]
                )
                + "\n"
            )
    ...
    def convert_inter(self):
        ...
        for i in tqdm(range(lines_count)):
            ...
            # Update the user_id
            if line_dict["user_id"] not in self.user_map:
                self.user_map[line_dict['user_id']] = len(self.user_map) + 1
            line_dict['user_id'] = self.user_map[line_dict['user_id']]
            if line_dict["business_id"] not in self.item_map:
                self.item_map[line_dict['business_id']] = len(self.item_map) + 1
            line_dict['business_id'] = self.item_map[line_dict['business_id']]
            if line_dict['business_id'] not in self.item_text_map:
                self.item_text_map[line_dict['business_id']] = []
            self.item_text_map[line_dict['business_id']].append(line_dict['business_id'])
            fout.write(
                "\t".join(
                    [
                        str(
    ...
    def write2file(self):
        ...
        with open(self.output_item2index_file, 'w') as f:
            for k, v in self.item_map.items():
                f.write(f"{k}\t{v}\n")
        with open(self.output_item2text_file, 'w') as f:
            for k in self.item_text_map.keys():
                f.write(f"{k}\t")
                for v in self.item_text_map[k]:
                    f.write(f"{v} ")
                f.write(f"\n")
    ...
    def convert_item(self):
        ...
        fout.write(
            "\t".join(
                [
                    self.item_fields[column]
                    for column in self.item_fields.keys()
                ]
            )
            + "\n"
        )
            ...
            fout.write(
                "\t".join(
                    [
                        str(
                            line_dict[
                                self.item_fields[key][
                                0: self.item_fields[key].find(":")
                                ]
                            ]
                        )
                        for key in self.item_fields.keys()
                    ]
                )
                + "\n"
            )
